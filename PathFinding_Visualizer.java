...
public class PathFinding {
	...
	private final int MSIZE = 400;
	private int CSIZE = MSIZE/cells;
	//UTIL ARRAYS
	private String[] algorithms = {"Dijkstra","A*","BFS","DFS"};
...
//	JLabel lengthL = new JLabel("Path Length: "+length);
	...
	private void initialize() {	//INITIALIZE THE GUI ELEMENTS
		...
		toolP.setBorder(BorderFactory.createTitledBorder(loweredetched,"Options"));
		int space = 20;
		int buff = 125;
		
		toolP.setLayout(null);
		toolP.setBounds(5,5,800,50);
		
		searchB.setBounds(space,20, 120, 25);
		toolP.add(searchB);
		space+=buff;
		
		resetB.setBounds(space,20,120,25);
		toolP.add(resetB);
		space+=buff;
//		
		genMapB.setBounds(space,20, 120, 25);
		toolP.add(genMapB);
		space+=buff;
//		
		clearMapB.setBounds(space,20, 120, 25);
		toolP.add(clearMapB);
		space+=buff;
		
//		algL.setBounds(space,20,120,25);
//		toolP.add(algL);
//		space+=buff;
		
		algorithmsBx.setBounds(space,20, 120, 25);
...
//		space+=buff;
//		
//		delayL.setBounds(space,20,50,25);
//		toolP.add(delayL);
...
//		
		canvas = new Map();
		canvas.setBounds(10, 60, 600, 600);
...
//		
	...
	class Algorithm {	//ALGORITHM CLASS
		...
		public void Dijkstra() {
			PriorityQueue<Node> priority=new PriorityQueue<>(new Comparator<Node>(){

				@Override
				public int compare(PathFinding.Node o1, PathFinding.Node o2) {
			...
			priority.add(map[startx][starty]);	//ADD THE START TO THE QUE
			while(solving) {
				if(priority.isEmpty() ) {	//IF THE QUE IS 0 THEN NO PATH CAN BE FOUND
					solving = false;
					break;
				}
